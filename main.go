...
func main() {
	...
	useTestnet := database.IsUsingTestnet()
	go market.NewWSMonitor(150, useTestnet).Start(database.GetCustomCoins())
	//go market.NewWSMonitor(150, useTestnet).Start([]string{}) //这里是一个使用方式 传入空的话 则使用market市场的所有币种
	// 设置优雅退出
