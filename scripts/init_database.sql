-- ============================================================
-- NoFX Trading System - PostgreSQL Database Initialization
-- ============================================================
-- 此脚本用于初始化所有必需的数据库表和索引
-- 使用方法: psql -U username -d database_name -f init_database.sql
-- ============================================================

-- 1. 系统级 AI 模型模板表（所有用户共享）
...
-- 为 traders 表添加 exchange_api_key_name 字段
DO $$ 
BEGIN 
    IF NOT EXISTS (
...
-- 为 decision_logs 表添加 system_prompt, input_prompt, cot_trace 字段
DO $$ 
BEGIN 
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
